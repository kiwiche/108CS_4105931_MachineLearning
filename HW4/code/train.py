...
def train():
    ...
    model = CNN(num_classes=train_set.num_classes)
    ...
    for epoch in range(1, num_epochs+1):
        ...
        print('%s (%d %d%%) Training loss: %.4f accuracy: %.4f' % (timeSince(start, epoch / num_epochs), epoch, epoch / num_epochs * 100, training_loss, training_acc))
        ...
        if epoch % 10 == 0:
            model_path = os.path.join("./save", f'model-{best_acc:.02f}-best_train_acc.pth')
